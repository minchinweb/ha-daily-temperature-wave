...
def main():
    ...
    for hours_from_noon, time_str in times:
        temp_c = calculate_temperature(
            min_temp_c, max_temp_c, hours_from_noon, wave_spread
        )
        temp_display = convert_from_celsius(temp_c, unit_system)

        # Calculate position in wave (0-100%)
        wave_position = (
            (math.sin(math.pi * hours_from_noon / (12 * wave_spread)) + 1) / 2 * 100
        )
        ...
        desc = (
            ...
            if spread <= 0.5
                ...
                if spread < 1.0
                    ...
                    "Standard"
                    if spread == 1.0
                    else "Steep" if spread < 2.0 else "Very Steep"
                )
            )
        )
